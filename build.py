...
hbb_name = 'olidesk' + ('.exe' if windows else '')
...
def make_parser():
    ...
    parser.add_argument(
        '--client',
        action='store_true',
        help='Build the stripped-down client build (receive-only, no settings)'
    )
    parser.add_argument(
        '--skip-cargo',
        action='store_true',
...
def generate_control_file(version):
    ...
    content = """Package: olidesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: olidesk <support@olisys.com>
Homepage: https://olisys.com
...
def build_flutter_deb(version, features):
    ...
    system2('dpkg-deb -b tmpdeb olidesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('olidesk.deb', '../olidesk-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/olidesk.exe')
    os.chdir('../..')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/olidesk.exe target/release/Olidesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\olidesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/Olidesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/Olidesk.app/Contents/MacOS/olidesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/Olidesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Olidesk.app/Contents/MacOS/olidesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Olidesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Olidesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/Olidesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/Olidesk.app
    '''.format(pa))
                system2(
                    'create-dmg "Olidesk %s.dmg" "target/release/bundle/osx/Olidesk.app"' % version)
                os.rename('Olidesk %s.dmg' %
    ...
    '''.format(pa, version))
                ...
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb olidesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('olidesk.deb', 'olidesk-%s.deb' % version)
