...
// Logging
//
// A verbose install (msiexec /l*v) of v1.4.45 showed NONE of this file's
// WcaLog() output -- not the HTTP status, not "got N group(s)", not a
// single insert result -- even though code that runs strictly after all of
// it (the MsiSetPropertyW call at the end of FetchGroups) demonstrably did
// execute (its own "PROPERTY CHANGE" line was in the log, generated by the
// MSI engine itself, not by us). So WcaLog's own output path is silently
// broken in this DLL for reasons unclear from source alone. LogInfo talks
// to MsiProcessMessage(INSTALLMESSAGE_INFO) directly instead -- the same
// primitive every verbose log line in the engine itself is built from --
// so it can't depend on WcaUtil's internal state working correctly.
...
bool HttpsGetJson(MSIHANDLE hInstall, const std::wstring& url, const std::wstring& bearerToken, std::string& outBody, DWORD& outStatusCode)
{
    bool ok = false;
    outStatusCode = 0;
    ...
    {
        LogInfo(hInstall, L"FetchGroups: failed to parse API URL, error %lu", GetLastError());
        return false;
    }
    ...
    if (!hSession)
    {
        LogInfo(hInstall, L"FetchGroups: WinHttpOpen failed, error %lu", GetLastError());
        return false;
    }
    ...
    if (!hConnect)
    {
        LogInfo(hInstall, L"FetchGroups: WinHttpConnect failed, error %lu", GetLastError());
    ...
    if (!hRequest)
    {
        LogInfo(hInstall, L"FetchGroups: WinHttpOpenRequest failed, error %lu", GetLastError());
    ...
    if (sent && WinHttpReceiveResponse(hRequest, NULL))
    {
        ...
        LogInfo(hInstall, L"FetchGroups: HTTP status %lu", statusCode);
    ...
    {
        LogInfo(hInstall, L"FetchGroups: request failed, error %lu", GetLastError());
    }
...
//
// v1.4.45 did this via literal parameterized SQL ("INSERT INTO `ComboBox`
// ... VALUES (?, ?, ?, ?)" / "DELETE FROM `ComboBox` WHERE ..." through
// MsiViewExecute) and it corrupted the table: a verbose log showed the
// resulting GROUP row's Value ending up as the *name of an unrelated
// property* we'd set elsewhere in the same custom action, never a real
// group name. ComboBox (like ListBox and RadioButtonGroup) is one of the
// runtime "list" tables the Windows Installer SDK explicitly documents as
// requiring MsiViewModify with MSIMODIFY_INSERT_TEMPORARY/MSIMODIFY_DELETE
// -- opened via a plain SELECT, not literal INSERT/DELETE SQL -- for
// reliable runtime population; the raw-SQL path apparently isn't safe for
// these specific tables even though the MSI SQL engine accepts it without
// error. Both helpers below now use that documented, supported pattern.
bool ClearComboBoxRows(MSIHANDLE hInstall, LPCWSTR property)
{
    ...
    UINT r = MsiDatabaseOpenViewW(hDb,
        L"SELECT `Property`, `Order`, `Value`, `Text` FROM `ComboBox` WHERE `Property` = ?",
        &hView);
    if (r != ERROR_SUCCESS) return false;

    PMSIHANDLE hParam = MsiCreateRecord(1);
    MsiRecordSetStringW(hParam, 1, property);

    r = MsiViewExecute(hView, hParam);
    if (r != ERROR_SUCCESS)
    {
        MsiViewClose(hView);
        return false;
    ...
    {
        MSIHANDLE hRowRaw = 0;
        UINT fr = MsiViewFetch(hView, &hRowRaw);
        if (fr != ERROR_SUCCESS)
        {
            break; // ERROR_NO_MORE_ITEMS -- done.
        }
        PMSIHANDLE hRow = hRowRaw;

        UINT mr = MsiViewModify(hView, MSIMODIFY_DELETE, hRow);
        if (mr != ERROR_SUCCESS)
        {
            ok = false;
        }
...
// Inserts one row into the ComboBox table for the given property. Uses
// MSIMODIFY_INSERT_TEMPORARY (not persisted to the .msi file, which is
// correct: these rows only need to exist for the current install session)
// via a view opened with SELECT, per Microsoft's documented pattern for
// populating ComboBox/ListBox/RadioButtonGroup at runtime -- see the
// comment above ClearComboBoxRows for why literal INSERT SQL was replaced.
bool InsertComboBoxRow(MSIHANDLE hInstall, LPCWSTR property, int order, const std::wstring& text)
{
    ...
    PMSIHANDLE hView;
    UINT r = MsiDatabaseOpenViewW(hDb,
        L"SELECT `Property`, `Order`, `Value`, `Text` FROM `ComboBox`",
        &hView);
    if (r != ERROR_SUCCESS) return false;

    r = MsiViewExecute(hView, NULL);
    if (r != ERROR_SUCCESS)
    {
        MsiViewClose(hView);
        return false;
    ...
    r = MsiViewModify(hView, MSIMODIFY_INSERT_TEMPORARY, hRecord);
    MsiViewClose(hView);
...
UINT __stdcall WriteDeployJson(__in MSIHANDLE hInstall)
{
...

    {
        ...
        // Never log deployToken itself -- everything below logs
        ...
        {
            LogInfo(hInstall, L"WriteDeployJson: API_URL or DEPLOY_TOKEN is empty, aborting.");
            goto LExit;
        }
        ...
        {
            LogInfo(hInstall,
                L"WriteDeployJson: GROUP looked like an MSI placeholder/status artifact ('%s'), treating as empty.",
                group.c_str());
            group.clear();
        }
